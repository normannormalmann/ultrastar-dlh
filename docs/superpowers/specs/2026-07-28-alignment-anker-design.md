...
**Nachtrag 2026-07-29 (Task 7) - Abschnitte durch Vierpass ersetzt:** Die Abschnitts-Maschinerie (Pass 1-2 Fenster-Aggregation, Ratenwaechter, Rand-Klemmung) ist obsolet: das Vierpass-Modell liefert praezisere Fenstergrenzen direkt aus den gemessenen Ankern (Pass 3), was uebrig bleibt wird interpoliert und mit Score 0,0 markiert (Pass 4). `Anchor`, `finde_anker`, `Abschnitt`, `baue_abschnitte` samt `MAX_WOERTER_PRO_SEKUNDE` und `RAND_SEKUNDEN_JE_WORT` sind entfernt; die echte LCS und die Messharness leben in `berechne_anker()` weiter.
